...
class aon_timer_scoreboard extends cip_base_scoreboard #(
  ...
  // Calculates number of clock cycles until an interrupt is raised. For the WKUP timer takes the
  // prescaler into consideration
  extern virtual task compute_num_clks();
  // Blocks when the 'pause_in_sleep' signal is set
  ...
  // Update WKUP/WDOG or both timed regs
  ...
  // Convenience wrapper function to avoid callind uvm_hdl_read multiple times
  extern function bit hdl_read_bit(string path);
  // Waits for the WE after a TL-UL write access
  extern task wait_for_we_pulse(input string path);
  // Predicts register wkup-cause considering reg.busy, and whether to wait for a write enable
  // if called due to a write to wkup-cause register
  extern task predict_wkup_cause(bit wkup_cause, bit wait_for_we = 0);
  // Predicts register intr_state considering reg.busy
  extern task predict_intr_state(bit [1:0] pred_intr_state);

  // Update functions waits for the value to cross CDC boundaries via checking the write enable
  ...
  //  RTL advice is to wrap the task around fork... join_none
  extern task wkup_intr_predicted_values(bit exp_wkup_intr);
  extern task wdog_intr_predicted_values(bit exp_wdog_intr);
  // Checks the predicted interrupt value is the current RTL value via backdoor reads
  ...
  // Convenience function to check if the wkup/wdog_ctrl.enable bits are un/set
      ...
      fork
        begin : wkup_cause_timely
          ...
          if (wait_for_we) begin
            bit we;
            do begin
              we = hdl_read_bit({path_to_rtl,".u_reg.aon_wkup_cause_we"});
              if (we === 0)
...
task aon_timer_scoreboard::wait_for_we_pulse(input string path);
  ...
  do begin
    we = hdl_read_bit({path_to_rtl, path});
    if (we === 0)
...
task aon_timer_scoreboard::wait_for_sleep();
  ...
  do begin
    synchronised_sleep_mode_i = hdl_read_bit(.path({path_to_rtl, ".aon_sleep_mode"}));
...
task aon_timer_scoreboard::collect_wkup_timer_coverage(ref event sample_coverage);
  forever begin
    wait (sample_coverage.triggered);
    if (cfg.en_cov) begin
      bit [63:0] rtl_count;
