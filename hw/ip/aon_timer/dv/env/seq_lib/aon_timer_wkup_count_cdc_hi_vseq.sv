...
task aon_timer_wkup_count_cdc_hi_vseq::wkup_count_hi_cdc_ccov_closure();
  ...
  // Otherwise, we'll likely won't hit the missing coverage holes for wkup_count_hi_cdc and arbiter
  cfg.lc_escalate_en_vif.drive(0);

  // Doing a random number of rounds because the signals we need to be hitting are crossing CDC and
  // the expectation is that by trying the combination below coverage will be hit
  for (int i = 0; i < $urandom_range(20,40); i++) begin
