...
module spi_host_data_stable_sva (
  ...
  input logic [3:0]                                         cio_sd_en_old_o,
  ...
  u_sva_cio_sd_en_o_whole_cycle_data_stable_check (
    ...
    whole_cycle_data_stable_signal_checker #(.VECTOR_WIDTH(4))
  u_sva_cio_sd_en_old_o_whole_cycle_data_stable_check (
     .rst_ni (rst_ni),
     .sck_o (cio_sck_o),
     .csb_o (csbs),
     .signal2check_i (cio_sd_en_old_o),
     .sampled_negedge_enable (sampled_negedge_enable),
     .sampled_posedge_enable (sampled_posedge_enable),
     .passthrough_en (passthrough_i.passthrough_en)
     );
...
(
 ...
 input logic                    passthrough_en
  ...
  property posedge_no_change_check_p;
    disable iff (rst_ni)
      @ (posedge sck_o) sampled_posedge_enable |-> !$changed(signal2check_i[i]);
  endproperty

  property negedge_no_change_check_p;
    disable iff (rst_ni)
      @ (negedge sck_o) sampled_negedge_enable |-> !$changed(signal2check_i[i]);
  endproperty

  NEGEDGE_SAME_VALUE_CHECK_P: assert property (negedge_no_change_check_p) begin
  ...
  POSEDGE_SAME_VALUE_CHECK_P: assert property (posedge_no_change_check_p) begin
